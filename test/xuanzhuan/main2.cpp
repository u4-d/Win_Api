...
void DrawTextInCircle(HDC hdc, const std::wstring &text, int centerX,
                      int centerY, int radius, int fontSize);
void DrawRotatingTextInCircle(HDC hdc, const std::wstring &text, int centerX,
                              int centerY, int radius, int fontSize,
                              bool isClockwise);
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    switch (uMsg) {
        ...
        case WM_PAINT: {
            ...
            // 绘制八卦文字和符号
            DrawRotatingTextInCircle(hdc, BAGUAWORDS, 400, 350, 300, 42,
                                     isClockwise);
            DrawRotatingTextInCircle(hdc, BAGUASYMBOLS, 400, 350, 250, 42,
                                     sysTrun);
            EndPaint(hwnd, &ps);
            break;
        }
...
// 函数：绘制旋转文字
void DrawRotatingTextInCircle_old(HDC hdc, const std::wstring &text,
                                  int centerX, int centerY, int radius,
                                  int fontSize, bool isClockwise) {
    int textLength = text.length();
    if (textLength == 0) return;
